                ...
                ,done: function(page, next){ //执行下一页的回调
                    ...
                    success : function(res) {
                            ...
                            layui.each(res.data, function(index, item){
                                html = '<a class="mdui-col" title="'+item.name+'" href="./?mod=buy&tid='+item.tid+'">';
                                html += '<div class="mdui-card">';
                                if(!item.shopimg){
                                    item.shopimg="https://www.dmoe.cc/random.php"
                                }
                                if(item.show_tag){
                                ...
                                if(item.is_stock_err == 1){
                                    shoukong = '<img  class="lazy" lay-src="./assets/store/picture/ysb.png" alt="" style="width:100%;top: 0;position: absolute;height:250px;">';
                                }
                                ...
                                html += ''+show_tag_html+'<div class="mdui-card-media"><img style="height:250px;" class="lazy" lay-src="'+item.shopimg+'" src="./assets/store/picture/loadimg.gif" alt="'+item.name+'">'+shoukong+'';
                                html += '';
                                

                                html += '<div class="mdui-card-media-covered mdui-card-media-covered-gradient">';
                               	html += '<div class="mdui-card-primary">';
                                html += '<div class="mdui-card-primary-title">'+item.name+'</div>';
                                html += '<div class="mdui-card-primary-subtitle">'+kucun+'</div></div></div></div>';
                                if(item.price <=0){
                                    buy = '<button style=" color: #FFF !important; " class=" mdui-float-right mdui-btn mdui-ripple mdui-color-green">领取</button>';
                                }else{
                                    buy = '<button class=" mdui-float-right mdui-btn mdui-ripple mdui-color-indigo">购买</button>';
                                }

                                if(item.stock == 0){
                                    buy = '<button class=" mdui-float-right mdui-btn mdui-ripple mdui-color-purple">缺货</div>';
                                }

								if(item.close == 1){
                                    buy = '<button class=" mdui-float-right mdui-btn mdui-ripple mdui-color-red">已下架</div>';
                                }

                                html += '<div class="mdui-card-actions">  <span class=" mdui-float-left mdui-btn mdui-ripple mdui-text-truncate">¥:'+item.price+'元</span>'+buy+'</div>';
                                html += '</div>';
                                html += '</a>';
                                lis.push(html);
                            });
